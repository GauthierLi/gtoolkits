...
def main(args: argparse.Namespace):
    ...
    # 检查是否是系统保留模块
    reserved_modules = {'create', 'remove', 'test_module', 'calculator'}
    ...
    if not args.force:
        print(f"\n⚠️  警告：此操作不可逆！")
        try:
            confirmation = input(f"确认删除模块 '{module_name}' 吗？[y/N]: ").strip().lower()
    ...
    if functions_dir.exists():
        for item in functions_dir.iterdir():
            if (item.is_dir() and 
                ...
                not item.name.startswith('__')):  # 排除 __pycache__ 等
